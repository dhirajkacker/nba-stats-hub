  ...
  useEffect(() => {
    async function loadTopPlayers() {
      try {
        ...
        const response = await fetch(`/api/player-stats-leaders?limit=30&season=${encodeURIComponent(season)}`);
        if (!response.ok) {
      ...
      <header className="bg-gradient-to-r from-gray-900 via-orange-600 to-gray-900 text-white shadow-2xl border-b-4 border-orange-500">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <p className="text-orange-200 mt-2 font-medium">
            {season} season leaders by {sortLabels[sortBy]}
          </p>
        </div>
      </header>
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
          <div className="p-6 bg-gradient-to-r from-orange-50 to-red-50 border-b border-gray-200">
            <div className="flex items-center gap-3">
              <div className="h-1 w-12 bg-gradient-to-r from-orange-500 to-red-500 rounded-full"></div>
              <h2 className="text-2xl font-black text-gray-900">{season} Season Leaders</h2>
            </div>
                ...
                <tbody className="divide-y divide-gray-100">
                  {sortedPlayers.map((player, index) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-4">
                        <Link href={`/seasons/${season}/players/${player.id}`} className="text-gray-900 hover:text-orange-600 transition-colors">
                      ...
                      <td className="py-4 px-4">
                        <Link
                          href={`/seasons/${season}/teams/${player.teamTricode.toLowerCase()}`}
